...
> 移植过程、三处有意差异与验收数据见 §十。**未做 / 已知限制**：
> **内层口令档（`V7_PASS`）不可用**（V6 骨架的 `read -p` 在 mksh 是"从 coprocess 读"
> ⇒ rc=1；外层口令档正常）、跨架构（aarch64）实测、`V7_WRAP` 接入（属 C4）。
...
> ⚠️ **上表的运行验证走的是「无内层口令档」。** 2026-09 文档追平期间的复验
> **新发现内层口令档（`V7_PASS` 非空）的 `read -p` 语义冲突**（V6 骨架的
> `IFS= read -rs -p 'Key: '`；mksh 的 `-p` = 从 coprocess 读）⇒ 该档 rc=1 不可用。
> 见 [`BUILD_MKSH.md`](BUILD_MKSH.md) §6.3 / §6.3b。
>
> **教训**：验收矩阵要按**档位组合**（口令档 × ISA 档 × 架构）设计，
> 而不是拿单个样例跑一遍 —— **10 项全过 ≠ 所有档位都过**。
...
| 项 | 状态 |
|---|---|
| **内层口令档（`V7_PASS`）不可用** | V6 骨架 `read -p` 与 mksh 语义冲突（mksh 的 `-p` = 从 coprocess 读）⇒ rc=1。**外层口令档正常**。修法候选：V6 生成器侧对目标解释器消歧 |
