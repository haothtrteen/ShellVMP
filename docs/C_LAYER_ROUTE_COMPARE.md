...
> ### 📌 本文的性质：**决策期的分析文档**（写作时 mksh 线尚未开工）
>
> **实测回顾**：上面的四项结论**基本被后续实施印证了**，其中两项被印证得比预期更好、
> 一项被推翻、一项未走。
>
> | 本文判断 | 实况 |
> |---|---|
> | ④「第一步给 mksh 做最小 PoC 验证抽象成立」 | ✅ 已走完（路线 C：C1/C2/C3，提交 `3829537`/`2c1098a`/`babf886`） |
> | ②「mksh 的保留字表更好插」 | ✅ **成立** —— `MKSH_R59C` 锚点集共 6 个 op 全部一次命中 |
> | ①「`isa_hook.c` 零 bash 依赖」 | ✅ 成立（这是移植成本可控的根本原因） |
> | ③「该投资「通用补丁」」 | ❌ **本轮未走**。用户明确选了"先把 mksh 移植做到底"（路线 C），通用补丁/`sh-hook` 子仓库**降级为 P3** |
> | A.7 的 6 天工时估算 | ⚠️ **偏乐观** —— 实际多出一个未预见的文件（`v7core.c` 包装层，见 A.5 末补记） |
>
> **当前实况**：mksh 线**已可一条命令打包产物**。
> 想要最新的状态，读 [`BUILD_PER_SHELL.md`](BUILD_PER_SHELL.md) §十 与
> [`BUILD_MKSH.md`](BUILD_MKSH.md) §九，**本文保留作决策记录**。
...
> **【实际落地修正，C1-C3 完成后补记】**
>
> 真正落地时比上面**多了 1 个文件**（共 4 个 `.c`，而非上文的 3 个）：
>
> | 文件 | 作用 |
> |---|---|
> | `v7_builtin_takeover.c` | builtin 接管（L6 出口） |
> | `v7_shf_inject.c` | **`shf_open()` 骨架注入（打开层劫持）** ← 上文漏了它 |
> | `v7core.c` | **`v7c_*` 包装层 —— 提供 8 个非 static 符号** |
> | `isa_hook.c` | ISA 四层翻译 |
>
> **为什么多一个 `v7core.c`**：`crypto_isa.h` 里的函数是 `static`
> （为让每个翻译单元各自内联一份），跨翻译单元**不可见** ⇒ `v7_shf_inject.c`
> 引用它们会得到 **8 条 `undefined reference`**。`v7core.c` 是薄包装层，把
> 需要的几张门面以非 static 符号导出。这是勘察阶段**没有预见**的。
>
> **另有一条硬约束**：C 文件**必须改名**进源树 ——
> `v7core_mksh.c → v7core.c`、`v7_builtin_takeover_mksh.c → v7_builtin_takeover.c`、
> `v7_shf_inject_mksh.c → v7_shf_inject.c`。锚点表按**改名后**的名字
> `#include` / 引用符号；不改名 = `cc1: fatal error: No such file or directory`。
> 源文件带 `_mksh` 后缀是为了在 `v7/mksh_poc/` 里与 bash 线同名文件区分。
>
> **还有一条**：`SRCS` 的幂等判据必须**逐项**判断（缺哪个加哪个），
> 不能"含任一文件就整体跳过"。本机不存在纯净 R59c 树（所有测试树都被污染），
> `SRCS` 实测有**三种形态**；用整体判据会在"局部状态"的树上静默漏加文件。
