...
compile_v5() {
    ...
    # 运行期 $- 恒为 hB（#!/usr/bin/env thirteen 非交互执行）；目标 bash 主版本默认取本机
...
prepare_passkeys() {
    ...
    if [ -z "$VM_MASTER_KEY" ]; then
        echo "错误：主密钥生成失败（openssl rand）" >&2
        return 1
    fi
...
__CK__=$__MK__
eval "$_D"
unset _D _iH _p1 _hs _e1 _e2 _M _m _f1 _f2 _f3 _f4 _f5 _v _fp _dv _ub _um _ut _uk _ok _try _ki _K _KS _PK _UI _nk 2>/dev/null
BOOTSTRAP_EOF
