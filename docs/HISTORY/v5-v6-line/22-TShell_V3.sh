...
# 设备绑定：HOST_BIND=1 时密钥掺入本机 serialno/android_id（产物仅本机可跑）
...
# 能力自检：编译机必须有支持 -aes-256-ctr -pbkdf2 的 openssl 和 gzip
check_crypto_capability() {
    ...
    if [ -z "$OPENSSL_BIN" ] || [ -z "$SHA512_BIN" ] || [ -z "$SHA256_BIN" ] || [ -z "$GZIP_BIN" ]; then
        echo "错误：本机缺少 openssl/gzip/sha512sum/sha256sum，无法生成 AES 版本。" >&2
        return 1
    fi
...
compile_v5() {
    ...
    local -a DETS=(
        ...
        "[ -x \"\$${N_OS}\" ] || exit 1; [ -x \"\$${N_S5}\" ] || exit 1; [ -x \"\$${N_S2}\" ] || exit 1"
    ...
    for ((i = 0; i < count; i++)); do
        ...
        VM_DMODES[$i]="$dmode"
        VM_DATA[$i]=$(aes_enc_item "$wrapped" "$data_key" "$dmode")
    ...
    if [ "$decoy_level" -ge 1 ]; then
        ...
        for ((dc = 0; dc < dc_total; dc++)); do
            ...
            dc_enc=$(aes_enc_item "$dc_wrapped" "$data_key" "${VM_DECOY_MODES[$dc]}")
            VM_DATA+=("$dc_enc")
    ...
    for ((b = 0; b < nb; b++)); do
        ...
        VM_INSTRUCTIONS+=("$(aes_enc_item "$bp" "$key" "$b")")
        sim_advance_mk "$key" "$b"
        key=$NEW_MK
    done

    echo "信息：V5编译完成，$count 真实块 / ${#VM_DATA[@]} 块总池，$total 条指令 / $nb 批（AES-256-CTR）" >&2
}
...
BOOTSTRAP_EOF
        ...
        # _SEED/_ITR/_hb/_GZ/_I 及 _hs/_p1/_fp/_dv 等为 bootstrap 私有
        # （先于任何用户代码执行，不可能被砸），保持字面名
        cat << 'BOOTSTRAP_EOF' | sed \
            -e "s/__OS__/${N_OS}/g" \
            -e "s/__S5__/${N_S5}/g" \
            -e "s/__MK__/${N_MK}/g" \
            -e "s/__CK__/${N_CK}/g"
...
_hs=${_hs:0:128}
_p1=$(printf '%s%s%s|%s' "$_SEED" "$_hs" "$_e1" "$_e2" | "$__S5__")
_p1=${_p1:0:64}
...
__CK__=$__MK__
eval "$_D"
unset _D _iH _p1 _hs _e1 _e2 _m _f1 _f2 _f3 _f4 _f5 _v _fp _dv 2>/dev/null
BOOTSTRAP_EOF
...
inject_integrity_hash() {
    ...
    encrypted_d0=$(aes_enc_item "$d0_real" "$VM_INITIAL_KEY" "${VM_DMODES[0]}")

    # 更新输出文件中的 N_D[0]
    # 用 awk 替换（AES base64 无 & / \，gsub 替换串安全；模式/前缀经 -v 传入）
    awk -v p="${N_D}\\[0\\]=\"[^\"]*\"" -v pre="${N_D}[0]=\"" -v nv="$encrypted_d0" \
        '{gsub(p, pre nv "\"")} 1' "$file" > "$file.tmp"
    mv "$file.tmp" "$file"
    chmod +x "$file"
...
# 一层壳封装（最后一步）
#
# P1 = sha512(_SEED + H_skel + 环境熵e1 + "|" + e2)[0:64]
...
finalize_layer1() {
    ...
    # P1 口令
    p1=$(printf '%s%s%s|%s' "$VM_KEY_SEED" "$hs" "$e1" "$e2" | "$SHA512_BIN" | cut -c1-64)
