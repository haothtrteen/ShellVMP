...
gen_randomized_interpreter() {
    ...
    local V_ENC V_HX V_BYT V_PC V_N V_INST V_CODE
...
#   - /system/build.prop 存在（安卓系统文件）
#   - ANDROID_ROOT 环境变量（安卓运行时设置）
#   - getprop 命令存在（安卓系统命令）
#   - /data/data/com.termux 存在（Termux 安装目录）
#   - TERMUX_VERSION / PREFIX 环境变量（Termux 运行时设置）
#   - /data/user/0 存在（安卓用户数据）
#
