...
#==============================================================================
# _s 执行状态计算器：密钥依赖运行时变量，必须真正执行才能得到下一个密钥
#==============================================================================

gen_s_computation() {
    local current_s="$1" di="$2"
    local pattern new_s code salt factor temp

    pattern=$((RANDOM % 4))
    case $pattern in
        0)
            code='_s=$(printf "%s%d" "$_s" "$_di" | sha256sum | cut -c1-8)'
            new_s=$(printf '%s%d' "$current_s" "$di" | sha256sum | cut -c1-8)
            ;;
        1)
            salt=$(gen_rand 4)
            code=$(printf '_s=$(printf "%%s%%s" "$_s" "%s" | sha256sum | cut -c1-8)' "$salt")
            new_s=$(printf '%s%s' "$current_s" "$salt" | sha256sum | cut -c1-8)
            ;;
        2)
            factor=$((RANDOM % 9 + 2))
            code=$(printf '_s=$(printf "%%s%%d" "$_s" "$((_di * %d))" | sha256sum | cut -c1-8)' "$factor")
            new_s=$(printf '%s%d' "$current_s" "$((di * factor))" | sha256sum | cut -c1-8)
            ;;
        3)
            code='_s=$(printf "%s" "$_s" | sha256sum | cut -c1-8); _s=$(printf "%s" "$_s" | sha256sum | cut -c1-8)'
            temp=$(printf '%s' "$current_s" | sha256sum | cut -c1-8)
            new_s=$(printf '%s' "$temp" | sha256sum | cut -c1-8)
            ;;
    esac
...
gen_ck_derivation_with_s() {
    ...
    local pattern new_ck code zz temp

    pattern=$((RANDOM % 4))
    case $pattern in
        0)
            code='_ck=$(printf "%s%s" "$_ck" "$_s" | sha256sum | cut -c1-16)'
            new_ck=$(printf '%s%s' "$current_ck" "$new_s" | sha256sum | cut -c1-16)
            ;;
        1)
            zz=$((RANDOM % 10000 + 1))
            code=$(printf '_zz=%d; _ck=$(printf "%%s%%s%%d" "$_ck" "$_s" "$_zz" | sha256sum | cut -c1-16)' "$zz")
            new_ck=$(printf '%s%s%d' "$current_ck" "$new_s" "$zz" | sha256sum | cut -c1-16)
            ;;
        2)
            code='_ck=$(printf "%s%s" "$_ck" "$_s" | sha256sum | cut -c1-16); _ck=$(printf "%s" "$_ck" | sha256sum | cut -c1-16)'
            temp=$(printf '%s%s' "$current_ck" "$new_s" | sha256sum | cut -c1-16)
            new_ck=$(printf '%s' "$temp" | sha256sum | cut -c1-16)
            ;;
        3)
            zz=$((RANDOM % 1000 + 1))
            code=$(printf '_zz=$((%d + _di)); _ck=$(printf "%%s%%s%%d" "$_ck" "$_s" "$_zz" | sha256sum | cut -c1-16)' "$zz")
            new_ck=$(printf '%s%s%d' "$current_ck" "$new_s" "$((zz + di))" | sha256sum | cut -c1-16)
            ;;
    esac
...
#==============================================================================
# 噪音块生成器：看起来像真代码的无意义计算
#==============================================================================

gen_noise_code() {
    local t=$((RANDOM % 5))
    case $t in
        0) echo '_zq=$(printf "%s" "$_s" | sha256sum | cut -c1-4); _zw=${#_zq}' ;;
        1) echo '_zq=$((_di * 13 + 7)); _zw=$((_zq / 2))' ;;
        2) echo '_zq=$(printf "%s" "$_di" | sha256sum | cut -c1-4); _zw=${#_zq}' ;;
        3) echo '_zq=$((_di * 3 + 1)); _zw=$((_zq + _zq))' ;;
        4) echo '_zq=$(printf "%s%s" "$_di" "$_di" | sha256sum | cut -c1-6); _zw=${_zq:0:3}' ;;
    esac
}
...
#   2. 噪音块与真实块交织，AI 无法区分哪些是真代码
#   3. 每个块（含噪音）都有不同的 _s 计算和 _ck 派生模式
#   4. 块数膨胀到 100+，AI 必须完整模拟每一步执行
#==============================================================================
...
compile_v5() {
    ...
    # 噪音比例
    local noise_ratio=${NOISE_RATIO:-4}

    # 构建块列表：噪音 + 真实交替
    local -a block_types=()
    for ((i = 0; i < count; i++)); do
        local n
        for ((n = 0; n < noise_ratio; n++)); do
            block_types+=("N")
        done
        block_types+=("R")
    done
    ...
    local total=${#block_types[@]}
    local data_key="$key"
    local det_idx=0 real_idx=0 data_idx=0

    for ((i = 0; i < total; i++)); do
        local btype="${block_types[$i]}"
        local wrapped det s_code new_s ck_code new_ck s_result ck_result content

        det_idx=$((RANDOM % ${#DETS[@]}))
        det="${DETS[$det_idx]}"

        s_result=$(gen_s_computation "$s" "$data_idx")
        s_code="${s_result%%$'\x01'*}"
        new_s="${s_result#*$'\x01'}"

        ck_result=$(gen_ck_derivation_with_s "$data_key" "$new_s" "$data_idx")
        ck_code="${ck_result%%$'\x01'*}"
        new_ck="${ck_result#*$'\x01'}"

        if [ "$btype" = "R" ]; then
            content="${REAL_BLOCKS[$real_idx]}"
            ((real_idx++))
        else
            content=$(gen_noise_code)
        fi
        ...
        VM_DATA[$data_idx]=$(encrypt_block "$wrapped" "$data_key")
    ...
    echo "信息：V5编译完成，$count 真实块 + $((data_idx - count)) 噪音块 = $data_idx 数据块，${#VM_INSTRUCTIONS[@]} 条指令" >&2
}
...
unset _mk _ck _p _n _s _di _zq _zw _zv _zz 2>/dev/null
INTERP_EOF
)
    ...
    {
        echo "#!/usr/bin/env bash @haothirteen"
        echo "# Generated: $timestamp"
        echo ""
