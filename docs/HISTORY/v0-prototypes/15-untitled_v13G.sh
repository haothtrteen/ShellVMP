...
gen_rand_var() {
    ...
    while :; do
        ...
        case "$name" in
            ...
            _am|_ra|_rb|_pm|_nw|_t0|_t|_pp|_rs|_B64|_SH256|_PRF|_I) continue ;;
            _da|_ke|_kb|_r|_i|_j|_b|_encoded|_hexdata|_bytes|_inst|_code|_ok|_pt) continue ;;
        esac
        for u in "${USED_VAR_NAMES[@]}"; do
            [ "$u" = "$name" ] && continue 2
        done
        USED_VAR_NAMES+=("$name")
        printf '%s' "$name"
...
gen_randomized_interpreter() {
    ...
    local V_AM V_RA V_RB V_PM V_NW V_T0 V_PP V_T
...
unset _mk _ck _p _n _s _di _rt _sa _sb _sc _sd _zz _zy _zv _rs _ra _rb _pm _nw _t0 _ok _pt _pp _t 2>/dev/null
I_EOF
)
    else
        interp=$(cat <<'I2_EOF'
...
compile_v5() {
    ...
    # 运行时公式（输出脚本内）: sha256(SEED + "$-" + VERSINFO[0] + 6)
    # 非真 bash（zsh/python模拟器假设错）→ 密钥直接错误，一步都解不开
    local key_seed
    key_seed=$(gen_rand 16)
    VM_KEY_SEED="$key_seed"
    key=$(printf '%s%s%d%d' "$key_seed" "hB" "$TARGET_BASH_MAJOR" 6 | sha256sum | cut -c1-16)
    ...
    for ((i = 0; i < count; i++)); do
        ...
        # 随机选择 EXEC 或 CEXEC(恒真谓词 0-3)，均带随机执行路径
        if [ $((RANDOM % 2)) -eq 1 ]; then
            inst_target="CEXEC:$i:$((RANDOM % 3)):$(gen_opaque_pred true)"
        else
            inst_target="EXEC:$i:$((RANDOM % 3))"
        fi
    ...
    if [ "$decoy_level" -ge 1 ]; then
        ...
        for ((dci = 0; dci < dc_total; dci++)); do
            inst_target="CEXEC:$((count + dci)):$((RANDOM % 3)):$(gen_opaque_pred false)"
            encrypted=$(encrypt_block "$inst_target" "$key")
            VM_INSTRUCTIONS+=("$encrypted")
            sim_advance_mk "$key" "$pos"; key=$NEW_MK; ((pos++))
        done
...
generate_output_v6() {
    ...
    # 功能4：耗时毒化阈值 = 指令数×3秒 + 30秒（微秒），慢设备余量充足，
    #        插桩/xtrace/单步调试会显著拖慢 → 触发毒化（密钥链静默歪掉，不报错）
    local interpreter tlimit
    tlimit=$(( ${#VM_INSTRUCTIONS[@]} * 3000000 + 30000000 ))
    interpreter="$(gen_randomized_interpreter "$tlimit")"
    ...
    local interp_key
    interp_key=$(printf '%s%s%d%d' "$all_c" "hB" "$TARGET_BASH_MAJOR" 6 | sha256sum | cut -c1-16)
...

    {
        ...
        # 初始密钥 + 执行状态变量
        # 功能3：_mk 运行时掺入 bash 变量派生（$- / BASH_VERSINFO），
        #        与编译时 key 初值公式逐比特一致；非真 bash → 密钥直接错
        # 功能1：_rs 是 $RANDOM 状态机种子，解释器 _am() 每条指令前用它重播种
        echo "_mk=\$(printf '%s%s%d%d' '$VM_KEY_SEED' \"\$-\" \"\${BASH_VERSINFO[0]}\" \"\${#BASH_VERSINFO[@]}\" | sha256sum | cut -c1-16)"
        echo "_ck=\$_mk"
...
_k=$(printf '%s%s%d%d' "$_a" "$-" "${BASH_VERSINFO[0]}" "${#BASH_VERSINFO[@]}" | sha256sum | cut -c1-16)
_r=""
