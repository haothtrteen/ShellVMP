...
simulate_execution() {
    ...
    # 在每块前用 true "__rr__" 重置 $_，确保模拟和运行时行为一致
    {
...
gen_s_computation() {
    ...
    local new_s
    new_s=$(eval "_s='$current_s'; _di=$di; _rt='$rt'; $code; printf '%s' \"\$_s\"")
...
gen_decoy_wrapped() {
    ...
    fake="$(gen_decoy_code)"
    s_r=$(gen_s_computation "dk_$(gen_rand 8)" "$di" "decoy")
    s_code="${s_r%%$'\x01'*}"
    new_s="${s_r#*$'\x01'}"
    ck_r=$(gen_ck_derivation_with_s "$key" "$new_s" "$di")
    ck_code="${ck_r%%$'\x01'*}"
    printf '%s\n%s\n%s\n%s\n%s\n%s' \
        "$det" 'true "__rr__"' "$fake" '_rt=$_' "$s_code" "$ck_code"
}
...
gen_randomized_interpreter() {
    ...
    I_DEC="_$(gen_rand 2)"; I_ENC="_$(gen_rand 2)"
    V_DA="_$(gen_rand 2)"; V_KE="_$(gen_rand 2)"; V_KB="_$(gen_rand 2)"
    V_R="_$(gen_rand 2)"; V_I="_$(gen_rand 2)"; V_J="_$(gen_rand 2)"; V_B="_$(gen_rand 2)"
    V_ENC="_$(gen_rand 2)"; V_HX="_$(gen_rand 2)"; V_BYT="_$(gen_rand 2)"
    V_PC="_$(gen_rand 2)"; V_N="_$(gen_rand 2)"; V_INST="_$(gen_rand 2)"; V_CODE="_$(gen_rand 2)"
    V_OK="_$(gen_rand 2)"; V_PT="_$(gen_rand 2)"
...
unset _mk _ck _p _n _s _di _rt _sa _sb _sc _sd _zz _zy _zv 2>/dev/null
I_EOF
)
    else
        interp=$(cat <<'I2_EOF'
...
# 注意：只用真实 Termux 上"必定存在"的信号，避免误杀：
#   - /system/build.prop（安卓系统文件，Termux 可读）
#   - /data/data/com.termux（Termux 安装目录）
#   - PREFIX（Termux 一定设置，指向 files/usr）
#   - TERMUX_VERSION（Termux 一定设置）
#   - /data/user/0（安卓用户数据目录）
# 不要用 ANDROID_ROOT（Termux shell 环境里没有）或 getprop（Termux 默认不装）。
#
...
gen_android_detection() {
    ...
    case $t in
        0)
            printf '%s' '[ -f /system/build.prop ] || exit 1; [ -n "${ANDROID_ROOT:-}" ] || exit 1'
            ;;
        1)
            printf '%s' 'command -v getprop >/dev/null 2>&1 || exit 1; [ -d /data/data/com.termux ] || exit 1'
            ;;
        2)
            printf '%s' '[ -n "${TERMUX_VERSION:-}" ] || exit 1; [ -n "${PREFIX:-}" ] || exit 1'
            ;;
        3)
            printf '%s' '[ -f /system/build.prop ] || exit 1; [ -d /data/user/0 ] || exit 1'
            ;;
    esac
}
...
compile_v5() {
    ...
    for ((i = 0; i < count; i++)); do
        ...
        s_result=$(gen_s_computation "$s" "$i" "$rt_val")
        s_code="${s_result%%$'\x01'*}"
        new_s="${s_result#*$'\x01'}"
        ...
        # 块结构：第一块含 完整性校验 + 安卓门控 + 检测 + 用户代码 + _s/ck派生
        if [ "$i" -eq 0 ]; then
            wrapped="${integ_code}"$'\n'"${android_gate}"$'\n'"${det}"$'\ntrue "__rr__"'$'\n'"${REAL_BLOCKS[$i]}"$'\n_rt=$_'$'\n'"${s_code}"$'\n'"${ck_code}"
    ...
    if [ "$decoy_level" -ge 1 ]; then
        ...
        for ((dc = 0; dc < dc_total; dc++)); do
            ...
            dc_enc=$(encrypt_block "$dc_wrapped" "$data_key")
            VM_DATA+=("$dc_enc")
    ...
    for ((i = 0; i < count; i++)); do
        ...
        # 随机选择 EXEC 或 CEXEC(恒真谓词 0-3)
        if [ $((RANDOM % 2)) -eq 1 ]; then
            inst_target="CEXEC:$i:$(gen_opaque_pred true)"
        else
            inst_target="EXEC:$i"
        fi
        ...
        case $pattern in
            0)
                encrypted=$(encrypt_block "$inst_target" "$key")
    ...
    if [ "$decoy_level" -ge 1 ]; then
        ...
        for ((dci = 0; dci < dc_total; dci++)); do
            inst_target="CEXEC:$((count + dci)):$(gen_opaque_pred false)"
            encrypted=$(encrypt_block "$inst_target" "$key")
            VM_INSTRUCTIONS+=("$encrypted")
            key=$(printf '%s%d' "$key" "$pos" | sha256sum | cut -c1-16); ((pos++))
        done
...
generate_output_v6() {
    ...
    # 构建 VM 解释器字符串（随机化变量名 + 控制流变体，将被整体加密）
    local interpreter
    interpreter="$(gen_randomized_interpreter)"
...
main() {
    ...
    local input_file="$1"
    local output_file="${2:-${input_file%.sh}_v5a.sh}"
