...
compile_v5() {
    ...
    # 第一个块额外注入自完整性校验（占位符，生成后替换为真实 hash）
    local data_key="$key"
    local det_idx=0

    # 完整性校验代码（占位符，inject_integrity_hash 会替换为真实 hash）
    # 原理：对 $0 做 sha256，但先用 sed 把 _d[0] 的值归一化为 "X"
    #       这样修改 _d[0] 的密文不影响 hash，而修改其他任何部分都会导致 hash 不匹配
    local integ_placeholder='@@INTEGRITY_HASH@@'
    local integ_code='[ -f "$0" ] && { _v=$(sed '\''s/_d\[0\]="[^"]*"/_d[0]="X"/'\'' "$0" | sha256sum | cut -c1-16); [ "$_v" = "@@INTEGRITY_HASH@@" ] || exit 1; }'

    for ((i = 0; i < count; i++)); do
        local wrapped det deriv_result deriv_code next_data_key
        det_idx=$((RANDOM % ${#DETS[@]}))
        det="${DETS[$det_idx]}"

        # 生成随机密钥派生模式（每个块不同，攻击者必须逐块分析）
        deriv_result=$(gen_key_derivation "$data_key")
        deriv_code="${deriv_result%%$'\x01'*}"
        next_data_key="${deriv_result#*$'\x01'}"

        if [ "$i" -eq 0 ]; then
            # 第一个块：完整性校验 + 检测 + 原始代码 + 密钥派生
            wrapped="${integ_code}"$'\n'"${det}"$'\n'"${REAL_BLOCKS[$i]}"$'\n'"${deriv_code}"
...
generate_output_with_e() {
    ...
    {
        echo "#!/usr/bin/env bash @皓thirteen 2026.8.12"
        echo "# ©️haothtrteen"
