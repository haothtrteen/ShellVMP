...
# -*- coding: utf-8 -*-
"""
discover_tables.py —— 规则 1 原型：自动发现解释器的"保留字表"

动机
================================================================================
`anchors.py` 里的插桩锚点目前**逐个 shell 手写**。手写成本高的根因是：
每个 shell 的"保留字表"叫什么名字、在哪个文件，事先不知道。
...
实测（2026-09）
================================================================================
    bash-5.2          → y.tab.c        word_token_alist  命中 19
    mksh-R59c         → syn.c          tokentab          命中 14
...
本文件是**原型**：只做发现，不做插桩，不改任何源码。产出供人工确认后写进
anchors.py 的角色映射表。
"""
import argparse
import glob
